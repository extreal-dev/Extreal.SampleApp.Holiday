...
{
    public class LightMultiplayHostPresenter : StagePresenterBase
    {
        private readonly AssetHelper assetHelper;
        private readonly NgoServer ngoHost;
        private readonly GameObject playerPrefab;

        private LightMultiplayHost multiplayHost;

        public LightMultiplayHostPresenter
        (
        ...
        {
            if (!appState.IsHost || !appState.IsLightForCommunication)
            {
                return;
            }

            multiplayHost = new LightMultiplayHost(ngoHost, playerPrefab, assetHelper);
        ...
        {
            if (!appState.IsHost || !appState.IsLightForCommunication || AppUtils.IsSpace(stageName))
            {
                return;
            }
