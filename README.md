...
- Create a `VivoxAppConfig` from the Create Assets menu in the `/Assets/Holiday/App` directory and set the Vivox access information in the inspector.
  - Path in the Create Assets menu: `Extreal > Integration.Chat.Vivox > VivoxAppConfig`
- Create a `MultiplayAppConfig` from the Create Assets menu in the `/Assets/Holiday/App` directory. Default values are set so there is no need to set them in the inspector.
    - Path in the Create Assets menu: `Extreal > Holiday > MultiplayAppConfig`
- Open `/Assets/Holiday/App/App` scene and set `VivoxAppConfig` and `MultiplayAppConfig` in the `Scope` object inspector.
